-- ============================================================================
-- LEGAL COMPLIANCE PACK — 2026-07-20
-- GDPR/DSA/consumer-law groundwork:
--   1) Terms/privacy acceptance (versioned, timestamped) + 18+ attestation
--   2) Public-preview opt-out honored by every anon-facing function
--   3) Right to erasure: delete_my_account() (full cascade + storage)
--   4) Right of access/portability: export_my_data() (JSON)
--   5) DSA notice-and-action: user_reports + report_user()
--   6) Consumer withdrawal function ("ångerknapp", SFS 2026:246): withdrawal_requests
--   7) One-click email unsubscribe (no login): unsubscribe_email(token)
--   8) Storage-limitation: 12-month purge of notification/email logs (pg_cron)
-- Idempotent: safe to re-run.
-- ============================================================================

-- ── 1) Consent + preview columns on profiles ────────────────────────────────
...
-- Record acceptance of the current Terms/Privacy version + adult attestation.
...
-- ── 2) Public preview opt-out: recreate every anon-facing function with the
--       filter. (public_board / public_players / top_* leaderboards) ─────────
...
language sql stable security definer set search_path = public as $$
  select id, name, photo_url, level, vibe::text,
   ...
   where coalesce(name, '') <> ''
     and public_preview            -- legal: opt-out honored
...
language sql stable security definer set search_path = public as $$
  select s.id, s.kind::text, s.play_at, s.created_at,
   ...
   where s.status = 'active'
     and s.play_at > now()
     and p.public_preview          -- legal: opt-out honored
...
-- Leaderboards are also anon-visible on /post — honor the same opt-out.
-- Bodies are verbatim copies of the live definitions + the public_preview
-- filter; signatures unchanged, so existing grants (incl. anon) carry over.
...
-- ── 3) Right to erasure — delete_my_account() ───────────────────────────────
-- Deletes the caller's auth user; FKs cascade to profiles, games, swipes,
-- buddies, kudos, push_subscriptions, sos_applications, event_attendees, etc.
-- Non-cascading references are nulled first; avatar files are removed from
-- storage; email logs tied to the address are erased (suppression list stays —
-- honoring an opt-out is a legal duty, and it holds no more than the address).
...
begin
  ...
  -- storage: avatar/photo objects live under "<uid>/…" in the avatars bucket.
  -- Best-effort: a storage-permission hiccup must never block the erasure
  -- itself (orphaned objects become unreachable once the signed URLs expire).
  begin
  ...
  -- null-out non-cascading references so the auth delete can't be blocked
  ...
  -- erase email trails for this address (keep suppressed_emails: legal duty)
...
-- ── 4) Right of access / portability — export_my_data() ────────────────────
...
-- ── 5) DSA notice-and-action — user reports ─────────────────────────────────
...
-- ── 6) Consumer withdrawal function (ångerknapp, distansavtalslagen via
--       SFS 2026:246 — in force since 19 June 2026) ─────────────────────────
...
declare _id uuid;
begin
  -- open to signed-in AND anonymous consumers: the law requires the function
  -- to be reachable where the contract was concluded, without hurdles.
...
-- ── 7) One-click email unsubscribe (no login required) ──────────────────────
...
-- ── 8) Storage limitation: purge notification/email logs after 12 months ───
