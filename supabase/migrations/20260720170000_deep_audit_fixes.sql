-- ═══════════════════════════════════════════════════════════════════
-- DEEP AUDIT FIXES 2026-07-20 (batch 5). Idempotent. Findings verified
-- against the effective (last-wins) function bodies.
-- ═══════════════════════════════════════════════════════════════════

-- ── P0 · claim_sos expired LIVE windowed games (same class as expire_old_sos;
--    re-killed the games the 07-20 resurrection brought back, on every tap) ──
...
-- ── SECURITY · profile privilege escalation: the freeze trigger guarded only
--    is_admin/signup_code. Any member could self-set member_tier='pro',
--    ghost_badge, rescues_count, etc. via a direct PostgREST update. Freeze the
--    whole privileged set for CLIENT roles (anon/authenticated); SECURITY
--    DEFINER RPCs run as the function owner (postgres) and pass through, so all
--    counter/tier logic keeps working untouched. Trigger is SECURITY INVOKER so
--    current_user reflects the real caller. Editable profile fields
--    (name/bio/sports/goals/email_notifs/...) are NOT in the list — unaffected.
...
-- ── SECURITY · non-admins could set ghost_name/ghost_claim_token on INSERT
--    (fake-host impersonation on the board). Gate ghost fields to admins. ──
...
-- ── P1 · cancel_game left pending applicants waiting forever (no notify, still
--    'pending'). Now also declines them and returns their ids to notify. ──
...
-- ── P2 · publish_my_game: let a host flip a private (invited) game onto the
--    board if the invitee never joins — previously a dead end (cancel+repost). ──
...
-- ── P2 · join_game_by_token had NO time guard (joinable after window end). ──
...
-- ── P2 · claim_ghost_game had no status/time guard (could revive a dead game
--    or self-pair if the token holder had already claimed the SOS). ──
...
BEGIN
  ...
  IF _sos.caller_id = _uid THEN RETURN QUERY SELECT false, 'own'::text; RETURN; END IF;
  IF _sos.status <> 'active' THEN RETURN QUERY SELECT false, 'not_active'::text; RETURN; END IF;
  -- Don't hand a ghost game to someone already playing in it (would self-pair).
...
-- ── P2 · active_sos_count undercounted live windowed games (a per-user SOS cap
--    could be exceeded). Window-aware. ──
...
-- ── P2 · edit_sos changed format but never recomputed spots_needed (set_spots_
--    needed is BEFORE INSERT only) → a singles→doubles edit closed the game with
--    1 of N players; doubles→singles never completed. Recompute on edit. ──
...
-- ── P2 · report_noshow revoked rescue credit only from the LAST claimer
--    (claimed_by). Revoke from the actual reported player when they were the
--    joiner/rescuer of THIS game (games.player_b), which is doubles-safe. ──
...
BEGIN
  ...
  -- rescue credit belongs to the joiner (player_b); revoke only if the no-show
  -- was that joiner and this game came from an SOS.
