-- ═══════════════════════════════════════════════════════════════════
-- DATA-DRIVEN CITIES (re-landed 2026-07-20; original work of 2026-07-04 was
-- lost in the TanStack Start rebuild). Add a city = INSERT a row, no code.
--  * public.cities (name/country/timezone/granularity_min/active/sort) + seeds
--  * courts-insert policy allows any ACTIVE city (was hardcoded Uppsala/Sthlm)
--  * sos_push_targets quiet hours use the SOS court city's timezone —
--    rebased onto the CURRENT body (sport matching + kind='sos' weekly cap)
--  * admin_dashboard by_city reads the cities table
-- Idempotent.
-- ═══════════════════════════════════════════════════════════════════
...
-- Seed a few public Miami courts so the city isn't empty on launch (re-run safe).
...
-- Court creation: any ACTIVE city instead of the hardcoded pair.
...
-- Quiet hours in the court city's own timezone (current body + tz CTE):
...
-- Admin dashboard by-city stats follow the cities table:
